...
A large fraction of theoretical guarantees in machine learning, including generalization bounds, regret bounds in online learning, and confidence intervals in bandit algorithms, rely on concentration inequalities for sums of random variables. In many of these results, randomness is controlled through a sub-Gaussian assumption, which provides exponential tail bounds with variance-like parameters. While this framework is remarkably general, the constants appearing in such bounds often play a crucial quantitative role, especially in non-asymptotic regimes, yet are frequently chosen in a conservative or distribution-agnostic manner.

The sub-Gaussian property, first characterized by @Kahane1960PropritsLD and @buldygin1980sub, formalizes this notion of light-tailed behavior via moment-generating function control. Since these pioneering works, sub-Gaussianity has become a cornerstone of learning theory, underpinning concentration inequalities [@Hoeffding1963; @boucheron2003concentration; @raginsky2013concentration], regret analyses in stochastic bandits [@zhou2024towards], bounds on random matrix spectra [@rudelson2010non], and uncertainty quantification in Bayesian neural networks [@vladimirova2019understanding; @vladimirova2020sub].

::: {#def-subgaussian}
## Sub-Gaussian variables

A random variable $X$ with finite mean $\mu = \mathbb{E}[X]$ is *sub-Gaussian* if there exists a constant $\sigma^2 > 0$ such that
$$
\mathbb{E}[\exp(\lambda (X - \mu))] \leq \exp(\lambda^2 \sigma^2/2)
\quad \text{for all } \lambda \in \mathbb{R}.
$$
Such a constant $\sigma^2$ is called a *variance proxy*, and the smallest admissible value,
$$
\sigma^2_{\mathrm{opt}}(X) = \inf \left\{ \sigma^2 > 0 \;:\; X \text{ is } \sigma^2\text{-sub-Gaussian} \right\},
$$
is referred to as the *optimal variance proxy*. A variance proxy is always lower bounded by the variance, and when equality holds, i.e., $\sigma^2_{\mathrm{opt}}(X) = \mathrm{Var}[X]$, the random variable is said to be *strictly sub-Gaussian*.
:::

From a learning-theoretic perspective, the optimal variance proxy directly governs the tightness of concentration bounds and therefore affects learning rates, exploration bonuses, and confidence widths. Despite this importance, most existing results on optimal variance proxies focus on continuous distributions, including Beta and Dirichlet laws [@marchal2017sub], other bounded distributions such as Kumaraswamy and triangular distributions [@arbel2020strict], as well as truncated Gaussian and exponential models [@barreto2024optimal].

In contrast, discrete distributions, which naturally arise in learning from categorical data, discretized rewards, count observations, and combinatorial stochastic processes, remain comparatively underexplored from the viewpoint of optimal sub-Gaussian constants. The canonical example is the Bernoulli distribution. For a Bernoulli random variable supported on $\{0,1\}$ with parameter $p$, @Kearns1998LargeDM derived the following tight inequality for the moment-generating function:
$$
(1-p)e^{-\lambda p}+pe^{\lambda(1-p)}\leq\exp\left(\frac{1-2p}{4\ln((1-p)/p)}\lambda^2\right),
\quad p\in[0,1],\,\,\lambda\in\mathbb{R},
$$ {#eq-KS}
which yields an explicit expression for the optimal variance proxy. This result has important implications in learning theory, as it immediately transfers to binomial sums and empirical averages of Bernoulli observations.

A natural question is whether such sharp characterizations extend beyond the Bernoulli case. More generally, discrete observations in learning are often modeled by categorical distributions with multiple atoms, yet obtaining tight sub-Gaussian parameters for such distributions appears challenging in full generality. In this work, we take a step in this direction by studying discrete distributions with three equally spaced support points. This setting is the minimal non-Bernoulli discrete model exhibiting nontrivial sub-Gaussian behavior, including phase transitions between strict and non-strict sub-Gaussianity, and admits both symmetric and asymmetric configurations. To the best of our knowledge, the general $N$-mass case appears to be largely intractable, except for the uniform case that we analyze here (see @fig-distrib-plots).

**Implications for learning and bandits.**
Beyond their intrinsic probabilistic interest, the results of this paper have direct consequences for learning-theoretic guarantees that rely on concentration inequalities. In many learning problems, including empirical risk minimization and stochastic bandits, noise is modeled as sub-Gaussian with a variance proxy that is chosen conservatively, often ignoring the fine structure of the underlying distribution. Our results show that, for discrete observations with few support points, this variance proxy can be characterized exactly, and in several regimes coincides with the variance itself.

As a concrete illustration, we show how the optimal sub-Gaussian variance proxies derived in this work can be plugged into standard confidence bounds for empirical means, leading to tighter confidence intervals for discrete-valued rewards. In the context of stochastic multi-armed bandits with discrete reward distributions, this yields sharper exploration bonuses and improved constants in regret bounds for classical algorithms, without altering their structure. This application highlights how refined probabilistic analysis of discrete noise models can translate into tangible improvements in learning-theoretic performance guarantees. Note that this refinement of the probabilistic layer underlying standard learning guarantees does not require algorithmic modifications.
...
We first provide in @sec-charac a general characterization of the optimal sub-Gaussian variance proxy for random variables with bounded moment-generating functions. This characterization leads to a practical computational procedure based on the identification of critical points of an associated variational function (@thm-GeneralCharac) with refined results when the number of critical points is small (@prp-g2zeros and @prp-LemmaMerged).

We then apply this framework to discrete distributions that naturally arise in learning-theoretic models. In @sec-3-mass, we analyze three-point distributions, extending the classical Bernoulli case. In the symmetric setting, @thm-TheoSymmetric3mass reveals a sharp phase transition: for probabilities $p \geq \frac{1}{6}$, the distribution is strictly sub-Gaussian, whereas for $p < \frac{1}{6}$ it is not, and the optimal variance proxy is characterized by an explicit system of equations. In the asymmetric case, we identify two regimes depending on the mass of the central atom, and derive closed-form expressions in one of them (@thm-regime1). In @sec-uniform, we further show that the discrete uniform distribution on $N$ equally spaced points is strictly sub-Gaussian for all $N \geq 2$.

Building on these results, we demonstrate in @sec-applications how the optimal variance
proxies derived in this work can be incorporated into standard learning-theoretic guarantees.
We show that plugging these proxies into classical concentration inequalities yields tighter
confidence bounds for empirical means, and we illustrate the impact of this refinement on regret
bounds for stochastic multi-armed bandits, without modifying the underlying algorithms.

Finally, we describe in @sec-software the open-source Python package we developed to support reproducibility, which combines analytical and numerical approaches to compute optimal sub-Gaussian variance proxies across a wide range of distributions, with direct applicability to learning-theoretic problems.
...
$g^{(2)}_Y(\lambda;\sigma^2) = \sigma^2 - M_Y^{(2)}(\lambda)$.
...
$\lim_{\lambda\to \pm\infty} g^{(2)}_Y(\lambda;\sigma^2) = \sigma^2$.
:::

::: {#fig-ImageTheorem2}
![](figures/illustration_thm21.png){width=100%}
...
$g_Y^{(2)}\!\left(\lambda;\sigma^2 = M_Y'(\lambda)/\lambda\right) > 0$).
...
The main numerical and theoretical difficulty in studying elements of
...
However, when $g_Y^{(2)}(.;\sigma^2)$ has very few zeros, it is possible to study
its sign and thus remove this complication.

::: {#prp-g2zeros}
## Case when $g_Y^{(2)}$ has at most one zero on a half-line

Assume that $M_Y$ is a smooth function and that
$M_Y(\lambda) \overset{\lambda\to \pm\infty}{=} o(\lambda^2)$.
Assume that $g_Y^{(2)}(.;\sigma^2)$ has no or one zero on $\mathbb{R}_+^*$ for some
...
$g_Y^{(2)}(.;\sigma^2)$ has more than one zero on a half-line. However, we can still
get information when $g_Y^{(2)}(.;\sigma^2)$ has exactly two zeros on a half-line.
Two cases are studied in @lem-numberA1 and @lem-numberA2 provided in appendix
@sec-appendix-A which can be put together to obtain the following
proposition.

::: {#prp-LemmaMerged}
## Case when $g_Y^{(2)}$ has exactly two zeros on a half-line

Assume that $M_Y$ is a smooth function and that
$M_Y(\lambda) \overset{\lambda\to \pm\infty}{=} o(\lambda^2)$.
Also assume that for any $\sigma^2 > 0$,
$g_Y^{(2)}(.;\sigma^2)$ has exactly two zeros
...
the optimal variance proxy when $g_Y^{(2)}(.;\sigma^2)$ has at most two zeros on each
...
the inequality above for $\lambda\geq 0$.
...
\end{aligned}
$$
Observe that for all $\lambda \in \mathbb{R}$,
...
![](figures/illustration_symmetric_case.png){width=78%}

![](figures/illustration_asym_limits.png){width=82%}
...
The proof is deferred to the appendix @sec-appendix.
...
![](figures/g_plot_p1p2_1325.png){width=80%}
...
The proof of @thm-discrete-distribution is postponed to appendix @sec-appendix-proof-uniform.

# Applications to learning-theoretic guarantees {#sec-applications}

This section illustrates how the optimal sub-Gaussian variance proxies characterized in the
previous sections can be directly leveraged in learning-theoretic guarantees. We focus on
concentration inequalities for empirical means and on regret bounds for stochastic multi-armed
bandits, two canonical settings in learning theory where sub-Gaussian assumptions play a central role.

## Sub-Gaussian concentration with optimal variance proxy {#subsec-concentration}

We begin by recalling a standard concentration inequality for empirical averages, emphasizing
the role of the variance proxy. While the result itself is classical, its interest here lies in the
fact that the variance proxy can be chosen optimally for the discrete distributions studied in
this paper.

::: {#thm-hoeffding-opt}
## Hoeffding-type concentration with optimal variance proxy

Let $X_1,\dots,X_n$ be i.i.d. random variables with mean $\mu$, such that each $X_i$ is
$\sigma^2_{\mathrm{opt}}$-sub-Gaussian.
Then, for all $\delta \in (0,1)$,
$$
\Pr\!\left(
\left| \frac{1}{n}\sum_{i=1}^n X_i - \mu \right|
\geq \sqrt{\frac{2\sigma^2_{\mathrm{opt}}\log(2/\delta)}{n}}
\right)
\leq \delta .
$$
:::

@thm-hoeffding-opt shows that any improvement in the sub-Gaussian variance proxy
translates directly into tighter confidence intervals for empirical means. In particular, when
$\sigma^2_{\mathrm{opt}}=\mathrm{Var}(X)$, the resulting bound is optimal up to universal constants.

## UCB with optimized sub-Gaussian parameter {#subsec-ucb}

We now turn to stochastic multi-armed bandits. Consider a $K$-armed bandit problem in which
each arm $k$ yields i.i.d. rewards with mean $\mu_k$ and sub-Gaussian tails. Classical analyses
of upper confidence bound (UCB) algorithms [@auer2002using; @garivier2011kl] assume a generic sub-Gaussian parameter, often derived from boundedness
assumptions. The next result highlights that the regret bound depends explicitly on the variance
proxy and therefore benefits from its optimal calibration.

::: {#thm-ucb-opt}
## Regret bound for UCB with optimal variance proxy

Consider a stochastic $K$-armed bandit problem in which rewards from arm $k$ are i.i.d. with
mean $\mu_k$ and are $\sigma_k^2$-sub-Gaussian, where $\sigma_k^2$ denotes the optimal variance
proxy of the reward distribution.
Then the regret $R_T$ of the UCB algorithm at horizon $T$ satisfies
$$
\mathbb{E}[R_T]
\leq
\sum_{k:\mu_k<\mu^\star}
\frac{C\,\sigma_k^2\log T}{\Delta_k}
+ O\!\left(\sum_{k=1}^K \Delta_k\right),
$$
where $\mu^\star=\max_k \mu_k$, $\Delta_k=\mu^\star-\mu_k$, and $C>0$ is a universal constant.
:::

@thm-ucb-opt follows from a standard analysis of UCB combined with
@thm-hoeffding-opt. It shows that sharper sub-Gaussian variance proxies lead to
improved constants in regret bounds, without any modification of the algorithm.

## Instantiation: discrete three-point reward models {#subsec-3mass-bandit}

We now instantiate the previous result for discrete reward distributions with three support
points, as studied in @sec-3-mass. This setting naturally arises in applications
where rewards are discretized or take a small number of categorical values. Three-mass bandits can model for instance ternary feedback systems common in industry, such as accept/neutral/reject signals. Refined sub-Gaussian constants directly translate into safer and more efficient exploration.

::: {#cor-3mass-ucb}
## UCB regret for three-point reward distributions

Assume that rewards take values in $\{-a,0,a\}$ and follow a three-point distribution with
probability masses $(p_{k,1},p_{k,2},p_{k,3})$ for arm $k$.
Then the regret bound of @thm-ucb-opt holds with
$$
\sigma_k^2 = \sigma^2_{\mathrm{opt}}(p_{k,1},p_{k,2},p_{k,3}),
$$
where $\sigma^2_{\mathrm{opt}}$ is given explicitly by
@thm-TheoSymmetric3mass and @thm-regime1.
In particular, in the strictly sub-Gaussian regime identified in
@thm-TheoSymmetric3mass, one has $\sigma_k^2=\mathrm{Var}(X_k)$.
:::

This corollary highlights that, for discrete reward models, the exploration bonus used by UCB
can be calibrated exactly from the underlying distribution, leading to tighter regret guarantees
than those obtained from generic boundedness arguments.

# Software implementation {#sec-software}

To support reproducibility and facilitate the use of both state-of-the-art and our theoretical results, we developed a Python package^[The package is available at <https://github.com/jarbel/sub-Gaussian-implementation> with comprehensive documentation, installation instructions, and usage examples.] 
...
$$
as stated in @thm-GeneralCharac. This characterization is particularly effective for discrete distributions with few support points, such as the 3-mass distribution, where it provides a tractable criterion for identifying candidate values of the variance proxy.
...
**Second**, $\lambda_m(\sigma)\to 0_+$ when we increase $\sigma$. In this case this is a contradiction because $g_Y(.,\sigma^2)$ is locally positive and convex in a positive neighborhood of $0$. Indeed, $g_Y(.,\sigma^2)$ must reach a positive local maximum on $(0,\lambda_m(\sigma))$ that we denote $\lambda_{\text{max}}(\sigma)$. By Rolle's theorem on $(0,\lambda_{\text{max}}(\sigma))$ and $(\lambda_{\text{max}}(\sigma),\lambda_m(\sigma))$, there exist at least two distinct values $(\lambda_1(\sigma),\lambda_2(\sigma))$ with $0<\lambda_1(\sigma)<\lambda_{\text{max}}(\sigma)<\lambda_2(\sigma)<\lambda_m(\sigma)$ such that $g_Y^{(2)}(\lambda_1(\sigma);\sigma^2)=g_Y^{(2)}(\lambda_2(\sigma);\sigma^2)=0$. Since $\lambda_m(\sigma)\to 0_+$, we must also have $\lambda_1(\sigma),\lambda_2(\sigma)\to 0_+$ and hence by continuity $g_Y^{(2)}(0,\sigma^2)\to 0$. But this is impossible since $g_Y^{(2)}(0;\sigma^2)=(\sigma^2-\operatorname{Var}[Y])>0$ is a positive, increasing function of $\sigma$.
...
If $g_Y^{(2)}(.;\sigma^2)$ has no zero on $\mathbb{R}_+^*$, then it is strictly convex on $\mathbb{R}_+^*$ and since $g_Y(0;\sigma^2)=g_Y'(0;\sigma^2)=0$, $g_Y(.;\sigma^2)$ is positive on $\mathbb{R}_+^*$. Similarly, if $g_Y^{(2)}(.;\sigma^2)$ has a unique zero on $\mathbb{R}_+^*$, then it cannot change sign at this zero, because $g_Y^{(2)}(0;\sigma^2)=\sigma^2-\operatorname{Var}[Y]\geq 0$ and $\underset{\lambda\to +\infty}{\lim}g_Y^{(2)}(\lambda;\sigma^2)=\sigma^2>0$. Hence, we conclude similarly that $g_Y(.;\sigma^2)$ is positive on $\mathbb{R}_+^*$.
:::

::: {#lem-numberA1}
## Local minimum when $g_Y^{(2)}(.;\sigma^2)$ has two positive zeros and $\sigma^2>\operatorname{Var}(Y)$

Assume that $M_Y$ is a smooth function and that $M_Y(\lambda)\overset{\lambda\to \pm\infty}{=}o(\lambda^2)$. Moreover, assume that for any $\sigma^2\in \left(\operatorname{Var}[Y],+\infty\right)$, $g_Y^{(2)}(.;\sigma^2)$ has exactly two positive zeros $(\lambda_1(\sigma),\lambda_2(\sigma))$ such that $0<\lambda_1(\sigma)<\lambda_2(\sigma)$. Then, $g_Y(.;\sigma^2)$ has at most one local minimum $\lambda_m(\sigma)$ on $\mathbb{R}_+^*$ and it is necessarily located in $\lambda_m(\sigma)\in\left(\lambda_1(\sigma),\lambda_2(\sigma)\right)$.
...
**Proof of @lem-numberA1.** The proof follows from a precise study of variations. Indeed, let us first notice that for any $\sigma^2>\operatorname{Var}[Y]$, we have that $g_Y(.;\sigma^2)$ is locally convex and positive around $\lambda=0$ since $g_Y^{(2)}(0;\sigma^2)=\sigma^2-\operatorname{Var}[Y]>0$. We also remind that $\underset{\lambda\to \pm \infty}{\lim} g_Y^{(2)}(\lambda;\sigma^2)=\sigma^2>0$ so that $g_Y(.;\sigma^2)$ is also convex at infinity. Thus, from the assumption that $g_Y^{(2)}(.;\sigma^2)$ has exactly two zeros $(\lambda_1(\sigma),\lambda_2(\sigma))$ on $\mathbb{R}_+^*$, we conclude that either $g_Y^{(2)}(.;\sigma^2)$ does not change sign at its zeros and in this case, $g_Y(.;\sigma^2)$ is strictly convex on $\mathbb{R}_+^*$ with $g_Y(0;\sigma^2)=0$ so that it is positive and increasing on $\mathbb{R}_+$, thus there is no solution of $g_Y'(.;\sigma^2)=0$ on $\mathbb{R}_+^*$. Or that $g_Y^{(2)}(.;\sigma^2)$ is necessarily positive on $(0,\lambda_1(\sigma))$, negative on $(\lambda_1(\sigma),\lambda_2(\sigma))$ and positive on $(\lambda_2(\sigma),+\infty)$. Thus, $g_Y'(.;\sigma^2)$ is increasing on $(0,\lambda_1(\sigma))$ and since $g_Y'(0;\sigma^2)=0$ it is positive on $(0,\lambda_1(\sigma))$. Then, $g_Y'(.;\sigma^2)$ is decreasing on $(\lambda_1(\sigma),\lambda_2(\sigma))$ and then increasing on $(\lambda_2(\sigma),+\infty)$. In particular, $g_Y'(.;\sigma^2)$ admits a unique local minimum at $\lambda=\lambda_2(\sigma)$ on $\mathbb{R}_+^*$. Consequently, if $g_Y'(\lambda_2(\sigma);\sigma^2)\geq 0$, then $g_Y(.;\sigma^2)$ is non-negative on $\mathbb{R}_+$ and thus since $g_Y(0;\sigma^2)=0$, we get that $g_Y(.;\sigma^2)$ is non-negative on $\mathbb{R}_+$. Alternatively, if $g_Y'(\lambda_2(\sigma);\sigma^2)<0$, then $g_Y'(.;\sigma^2)$ has exactly two zeros $\lambda_1(\sigma)<\lambda_l(\sigma)<\lambda_r(\sigma)<\lambda_2(\sigma)$ and it is negative on $(\lambda_l(\sigma),\lambda_r(\sigma))$ and positive on $(0,\lambda_l(\sigma))\cup(\lambda_r(\sigma),+\infty)$. Consequently, $g_Y(.;\sigma^2)$ has exactly one local minimum on $\mathbb{R}_+^*$ denoted $\lambda_m(\sigma)$ which is necessarily located in $(\lambda_1(\sigma),\lambda_2(\sigma))$.
Next, let us observe that:
$$\partial_\sigma[g_Y'(\lambda_2(\sigma);\sigma^2)]=g_Y^{(2)}(\lambda_2(\sigma);\sigma^2) \partial_\sigma[\lambda_2(\sigma)]+ 2\sigma\lambda_2(\sigma)=2\sigma\lambda_2(\sigma)>0.$$
