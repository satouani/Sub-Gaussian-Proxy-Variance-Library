...
inequalities [@Hoeffding1963; @boucheron2003concentration; @raginsky2013concentration]
...
Bayesian neural networks [@vladimirova2019understanding; @vladimirova2020sub].
...
Research on optimal variance proxies has focused on continuous
...
distribution, the simplest discrete distribution, supported on two atoms,
$1$ and $0$, with masses $p$ and $1-p$. @Kearns1998LargeDM derived
...
@prp-g2zeros and @prp-LemmaMerged). We then apply this approach with a focus on discrete distributions. We start
in Section \ref{sec-3-mass} with 3-point distributions, both symmetric and asymmetric, extending
...
expression for the optimal proxy (@thm-TheoRegime2). We establish in Section \ref{sec-uniform} that discrete uniform distribution over $N$ equally
...
(@thm-discrete-distribution). Finally, we describe in Section \ref{sec-software} the computational framework we developed
